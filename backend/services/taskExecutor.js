        ...
        if (action.type === 'skill_call' && action.skill && action.skill.startsWith('agent:')) {
            ...
            switch (action.type) {
              case 'skill_call':
                ...
                break;
              case 'task_transfer':
                // 统一委托逻辑，调用包含 agent_id 保留修复的 delegateTask 方法
                await this.delegateTask(task, action);
                
                // 更新当前步骤的状态
                await dbRun(
                  'UPDATE task_steps SET status = ?, finished_at = ?, action_results = ? WHERE task_id = ? AND step_id = ?',
                  ['completed', new Date().toISOString(), JSON.stringify([{ action, result: { success: true, message: 'Task delegated' } }]), taskId, stepId]
                );
                // 返回正确的类型以结束当前执行链，防止被旧的、有问题的逻辑路径捕获
                return { type: 'delegation', details: action };
              default:
